...
global muted,speaking,speakingwhilemuted,r,g,b,w,ownid                                                       #define global variables
muted=speaking=speakingwhilemuted=r=g=b=w=0                                                                  #set all of them to 0
ownid = -1                                                                                                   #init the variable with number that isn't used by TeamSpeak
...
class eventlog(ts3plugin):
  ...
  def callback(self, name, *args):
    ...
    global r,g,b,w,speaking,muted,speakingwhilemuted,ownid                                                       #variables from the last call are deleted, so I used global ones
    ...
    if name == "onTalkStatusChangeEvent":                                                                        #track how many people are speaking
      ...
      if args[1] == 0:                                                                                             #if someone stops speaking
        if speakingwhilemuted and args[3] == ownid:                                                                  #if you stop speaking while muted
          speakingwhilemuted = 0                                                                                       #reset the variable
        else:                                                                                                        #else
      ...
      if args[1] == 2:                                                                                             #if you start speaking while muted but have not muted your speakers
        ownid = args[3]                                                                                              #save ownid
        speakingwhilemuted = 1                                                                                       #set variable
        ...
        s.connect((host, 5577))                                                                                      #connect to stripe
        prog = 49                                                                                                    #program for Stripe (36...57), 49 = red stroboscope
        delay = 1                                                                                                    #speed of program (1-31), 1 = fastest
        check = (0x70+prog+delay)%256                                                                                #calculate checksum
        s.send((chr(0x61)+chr(prog)+chr(delay)+chr(0x0f)+chr(check)).encode('raw_unicode_escape'))                   #send data to stripe
        s.close()                                                                                                    #end connection to stripe
        muted = 1                                                                                                    #set muted to 1, because mic is muted but not the speakers
    ...
    if changed and not speakingwhilemuted:                                                                       #if there may be a change and you are not speaking while your mic is muted
      ...
      s.settimeout(0.25)                                                                                           #use timeout, so TeamSpeak wouldn't crash if stripe is offline
      s.connect((host, 5577))                                                                                      #connect to stripe
      s.send((chr(0x31)+chr(r)+chr(g)+chr(b)+chr(w)+chr(0x0f)+chr(0x0f)+chr(check)).encode('raw_unicode_escape'))  #send data to stripe
      s.close()                                                                                                    #end connection to stripe
